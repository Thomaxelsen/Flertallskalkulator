// js/map-explorer.js (Versjon 4 - Tilbake til fungerende datalasting, håndterer mulig wrapper)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function loadData() {
        ...
        .then(([candidates, parties, geoJson]) => {
            console.log("Map Explorer JS: All data fetched successfully."); // VIKTIG LOGG
            allCandidatesData = candidates;
            geoJsonData = geoJson; // Lagrer rå geojson-respons
        ...
        .catch(error => {
            ...
            // Unngå å skrive over kart-containeren hvis feilen skjer ETTER at kartet er initialisert
             if (mapContainer && !map) { // Skriv bare feilmelding hvis kartet IKKE er laget ennå
                ...
                // Hvis kartet er laget, vis feil et annet sted eller logg bare
    ...
    function initMapExplorer() {
        ...
        // Fjernet: mapContainer.innerHTML = ''; // IKKE tøm her, kan skjule feilmeldinger fra fetch
        if (map) { map.remove(); map = null; } // Fjerner gammelt kart-objekt hvis det finnes
        ...
        // *** VIKTIG: Håndter mulig ekstra nivå i GeoJSON ***
        let featureCollectionData = null;
        if (geoJsonData) {
            if (geoJsonData.type === "FeatureCollection") {
                featureCollectionData = geoJsonData; // Standard GeoJSON
                ...
                featureCollectionData = geoJsonData.Valgdistrikt; // Håndterer filstrukturen fra Filen.txt
            }
        }
        // *** SLUTT PÅ HÅNDTERING ***

        if (featureCollectionData && featureCollectionData.features) { // Bruk featureCollectionData her
            console.log(`Map Explorer JS: Adding GeoJSON layer with ${featureCollectionData.features.length} features...`); // VIKTIG LOGG
            geoJsonLayer = L.geoJSON(featureCollectionData, { // Bruk featureCollectionData her
            ...
            if (geoJsonLayer.getBounds().isValid()) {
                ...
                console.log("Map Explorer JS: Map bounds fitted to GeoJSON."); // VIKTIG LOGG
            } else {
            ...
            console.error("Map Explorer JS: GeoJSON data is missing, invalid, or structure is unexpected.", geoJsonData);
             if (mapContainer) mapContainer.innerHTML = `<p class="error" style="padding: 20px;">GeoJSON-data mangler, er ugyldig eller har uventet struktur.</p>`; // Oppdater feilmelding hvis data mangler her
        }
    }
    ...
    function styleFeature(feature) {
        console.log("Styling feature:", feature?.properties?.valgdistriktsnavn || 'Ukjent'); // VIKTIG LOGG
        return {
    ...
    function zoomAndShowCandidates(e) {
        ...
        // Bruker nå CSS-klassen for valgt stil
         ...
         layer.bringToFront(); // Ta valgt lag fremst
        selectedLayer = layer;
        ...
        } else { console.error("Map Explorer JS: Could not find 'valgdistriktsnavn'!", layer.feature.properties); }
    }
    ...
    function displayCandidatesForConstituency(constituencyName) {
        ...
        if (panelTitle) { panelTitle.textContent = `${constituencyName} ${typeof mandateCount === 'number' ? '(' + mandateCount + ' mandater)' : ''}`; }
        listContent.innerHTML = ''; // Tøm
        if (!constituencyData || !constituencyData.parties || constituencyData.parties.length === 0) {
            listContent.innerHTML = '<p>Fant ingen kandidatdata for denne valgkretsen.</p>'; return;
        }
