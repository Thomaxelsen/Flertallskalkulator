...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function setupPanelEventListeners() {
    if (closePanelBtn)
      closePanelBtn.addEventListener("click", hideIssueDetails);
  ...
  function showIssueDetails(issueDataFromChart) {
    ...
    partiesData.forEach((party) => {
      const stance = fullIssue.partyStances
        ? fullIssue.partyStances[party.shorthand] || { level: 0 }
        : { level: 0 };
    ...
    let html = `<p class="issue-area">${
      fullIssue.area || "Ukjent saksområde"
    }</p>`;
  ...
  function createPartyListHTML(partiesArray, title, className) {
    ...
    let listItems = partiesArray
      .map((party) => {
        const profileUrl = `party-profile.aspx?party=${encodeURIComponent(
          party.shorthand
        )}`;
        return `
            <div class="stance-party-item">
                <a href="${profileUrl}" class="party-logo-link" aria-label="Les mer om ${
          party.name
        }">
                    <img src="images/parties/${party.shorthand.toLowerCase()}.png" alt="${
          party.name
        }" class="party-logo">
                </a>
  ...
  function processIssueData(supportLevelType) {
    if (
      ...
      issuesData.length === 0 ||
      Object.keys(partiesMap).length === 0
    )
      return [];
      ...
      if (issue.partyStances) {
        for (const partyCode in issue.partyStances) {
          ...
          if (partyInfo && stance && typeof stance.level !== "undefined") {
            ...
            let includeParty =
              (supportLevelType === "level-2" && level === 2) ||
              (supportLevelType === "level-1-2" &&
                (level === 1 || level === 2));
  ...
  function applyFiltersAndSort(processedIssues) {
    ...
    let filtered =
      areaFilter !== "all"
        ? processedIssues.filter((issue) => issue.area === areaFilter)
        : processedIssues;
    switch (sortFilter) {
      case "mandates_asc":
        return filtered.sort((a, b) => a.totalMandates - b.totalMandates);
      case "name_asc":
        return filtered.sort((a, b) => a.name.localeCompare(b.name));
      case "name_desc":
        return filtered.sort((a, b) => b.name.localeCompare(a.name));
      default:
        return filtered.sort((a, b) => b.totalMandates - a.totalMandates);
    }
  }
  function processAndVisualizeData() {
    const supportLevelType = document.getElementById(
      "sk-support-level-filter"
    ).value;
    ...
    setTimeout(() => {
      ...
      if (!finalData || finalData.length === 0) {
        visContainer.html(
          '<p class="no-data">Ingen saker funnet for gjeldende filtre.</p>'
        );
        updateLegend([]);
        return;
      }
  ...
  function createHorizontalBarChart(data) {
    ...
    let dynamicMarginLeft =
      containerWidth < 500 ? 150 : containerWidth < 768 ? 200 : 300;
    ...
    const tempYAxis = tempG
      .call(d3.axisLeft(tempYScale))
      .call((g) =>
        g.selectAll(".tick text").call(wrapAxisText, margin.left - 15)
      );
    ...
    const yScale = d3
      .scaleBand()
      .domain(data.map((d) => d.name))
      .range([0, height]); // Fjerner padding for nøyaktig posisjonering
    ...
    yAxis
      ...
      .attr(
        ...
        (d, i) =>
          `translate(0, ${
            calculatedPositions[i].y + calculatedPositions[i].height / 2
          })`
      );

    g.append("line")
      .attr("class", "majority-line")
      .attr("x1", xScale(MAJORITY_THRESHOLD))
      .attr("x2", xScale(MAJORITY_THRESHOLD))
      .attr("y1", -5)
      .attr("y2", height);
    g.append("text")
      .attr("class", "majority-label")
      .attr("x", xScale(MAJORITY_THRESHOLD))
      .attr("y", -8)
      .text(`Flertall`);
    ...
    const barGroups = g
      ...
      .on("mouseover", (event, d) => {
        ...
        yAxis
          .selectAll(".tick")
          .classed("highlighted", (tick_d) => d.name === tick_d);
      })
      .on("mouseout", () => {
    ...
    barGroups
      ...
      .attr(
        ...
        (d, i, nodes) =>
          calculatedPositions[
            data.findIndex((issue) => issue.name === d.issueName)
          ].height
      )
      ...
      .on("mouseover", (event, d) => {
        tooltip
          ...
          .html(
            `<b>${d.name}</b>Mandater: ${d.seats}<br>Støtte: Nivå ${d.level}`
          );
    ...
    yAxis
      ...
      .on("mouseover", (event, d_name) => {
        ...
        barGroups.classed(
          "highlighted",
          (bar_d) => correspondingData && bar_d.id === correspondingData.id
        );
        yAxis
          .selectAll(".tick")
          .classed("highlighted", (tick_d) => d_name === tick_d);
      })
      .on("mouseout", () => {
        container.classed("is-interacting", false);
        barGroups.classed("highlighted", false);
  ...
  function createDotPlot(data) {
    /* ... Den originale, komplette koden for dot plot ... */
  }
  function createTable(data) {
    /* ... Den originale, komplette koden for tabell ... */
  }

  function updateLegend(partyList) {
    const legendContainer = d3.select("#sk-legend-container");
    legendContainer.html("");
    const sortedParties = [...(partyList || [])].sort(
      (a, b) => (a.position || 99) - (b.position || 99)
    );
